...
import type { DripversityTestTypes } from './sources/dripversityTest/types';
import type { Uniswapv3PolygonTypes } from './sources/uniswapv3Polygon/types';
...
export type AccountResolvers<ContextType = MeshContext, ParentType extends ResolversParentTypes['Account'] = ResolversParentTypes['Account']> = ResolversObject<{
  ...
  authorizedUsers?: Resolver<Maybe<Array<ResolversTypes['Account']>>, ParentType, ContextType, RequireFields<AccountauthorizedUsersArgs, 'skip' | 'first'>>;
  collection?: Resolver<Array<ResolversTypes['Token']>, ParentType, ContextType, RequireFields<AccountcollectionArgs, 'skip' | 'first'>>;
  creations?: Resolver<Array<ResolversTypes['Token']>, ParentType, ContextType, RequireFields<AccountcreationsArgs, 'skip' | 'first'>>;
...
export type ContractResolvers<ContextType = MeshContext, ParentType extends ResolversParentTypes['Contract'] = ResolversParentTypes['Contract']> = ResolversObject<{
  ...
  maxReserved?: Resolver<Maybe<ResolversTypes['BigInt']>, ParentType, ContextType>;
  paused?: Resolver<Maybe<ResolversTypes['Boolean']>, ParentType, ContextType>;
  tokens?: Resolver<Array<ResolversTypes['Token']>, ParentType, ContextType, RequireFields<ContracttokensArgs, 'skip' | 'first'>>;
  __isTypeOf?: IsTypeOfResolverFn<ParentType, ContextType>;
}>;
...
export type TokenResolvers<ContextType = MeshContext, ParentType extends ResolversParentTypes['Token'] = ResolversParentTypes['Token']> = ResolversObject<{
  ...
  approved?: Resolver<Maybe<ResolversTypes['Account']>, ParentType, ContextType>;
...
export type MeshContext = DripversityTestTypes.Context & Uniswapv3PolygonTypes.Context & BaseMeshContext;
...
const importFn: ImportFn = <T>(moduleId: string) => {
  ...
  switch(relativeModuleId) {
    case ".graphclient/sources/dripversityTest/introspectionSchema":
      return import("./sources/dripversityTest/introspectionSchema") as T;
    
    case ".graphclient/sources/uniswapv3Polygon/introspectionSchema":
      return import("./sources/uniswapv3Polygon/introspectionSchema") as T;
...
const uniswapv3PolygonTransforms = [];
const dripversityTestTransforms = [];
const additionalTypeDefs = [] as any[];
const uniswapv3PolygonHandler = new GraphqlHandler({
              name: "uniswapv3Polygon",
              config: {"endpoint":"https://api.thegraph.com/subgraphs/name/ianlapham/uniswap-v3-polygon"},
              baseDir,
              cache,
              pubsub,
              store: sourcesStore.child("uniswapv3Polygon"),
              logger: logger.child("uniswapv3Polygon"),
              importFn,
            });
const dripversityTestHandler = new GraphqlHandler({
              name: "dripversityTest",
              config: {"endpoint":"https://api.thegraph.com/subgraphs/name/whitelabel-market/dripversity-test"},
              baseDir,
              cache,
              pubsub,
              store: sourcesStore.child("dripversityTest"),
              logger: logger.child("dripversityTest"),
              importFn,
            });
sources[0] = {
          name: 'uniswapv3Polygon',
          handler: uniswapv3PolygonHandler,
          transforms: uniswapv3PolygonTransforms
        }
sources[1] = {
          name: 'dripversityTest',
          handler: dripversityTestHandler,
          transforms: dripversityTestTransforms
        }
  ...
  return {
    ...
    get documents() {
      return [
      {
        ...
        get rawSDL() {
          return printWithCache(ContractDocument);
        },
        location: 'ContractDocument.graphql'
      },{
        document: TokensFromBlockDocument,
        get rawSDL() {
          return printWithCache(TokensFromBlockDocument);
        },
        location: 'TokensFromBlockDocument.graphql'
      },{
...
export type contractQuery = { contract?: Maybe<Pick<Contract, 'id' | 'name' | 'paused' | 'symbol' | 'maxSupply' | 'maxReserved' | 'totalSupply'>> };

export type tokensFromBlockQueryVariables = Exact<{
  address?: InputMaybe<Scalars['String']>;
  block?: InputMaybe<Scalars['BigInt']>;
}>;


export type tokensFromBlockQuery = { tokens: Array<(
    Pick<Token, 'id' | 'imageURI' | 'createdAtTimestamp' | 'createdAtBlockNumber' | 'transactionHash'>
    & { owner: Pick<Account, 'id'>, contract: Pick<Contract, 'id'> }
  )> };
...
export type tokenHourDatasQuery = { tokenHourDatas: Array<Pick<TokenHourData, 'periodStartUnix' | 'high' | 'low' | 'open' | 'close'>> };
...
export const tokensFromBlockDocument = gql`
    query tokensFromBlock($address: String = "", $block: BigInt = 0) {
  tokens(where: {owner: $address, createdAtBlockNumber: $block}) {
    ...
    ` as unknown as DocumentNode<tokensFromBlockQuery, tokensFromBlockQueryVariables>;
export const tokenHourDatasDocument = gql`
    query tokenHourDatas($first: Int! = 1, $skip: Int! = 0, $address: String! = "0x0d500b1d8e8ef31e21c99d1db9a6444d3adf1270") {
  tokenHourDatas(
    first: $first
    where: {token: $address}
    skip: $skip
...
export function getSdk<C, E>(requester: Requester<C, E>) {
  return {
    ...
    tokensFromBlock(variables?: tokensFromBlockQueryVariables, options?: C): Promise<tokensFromBlockQuery> {
      return requester<tokensFromBlockQuery, tokensFromBlockQueryVariables>(tokensFromBlockDocument, variables, options) as Promise<tokensFromBlockQuery>;
    },
